"""
=============================================================================
HYPERPARAMETER TUNING WITH ReliefF FEATURE SELECTION
=============================================================================
Script ini menggunakan ReliefF untuk seleksi fitur,
kemudian membandingkan 6 classifier dengan GridSearchCV.

ReliefF:
- Algoritma berbasis instance yang mengevaluasi fitur berdasarkan
  kemampuan membedakan instance dekat dari kelas berbeda (near-miss)
  vs kelas sama (near-hit)
- MULTIVARIAT - memperhitungkan interaksi antar fitur
- Cocok untuk data dengan interaksi fitur / nonlinearitas
- Memerlukan package: skrebate (pip install skrebate)
...
try:
    ...
    print("⚠️  WARNING: skrebate not installed!")
    print("   Install dengan: pip install skrebate")
    print("   Akan menggunakan fallback SelectKBest dengan f_classif")
...
# ReliefF specific parameters
RELIEFF_N_NEIGHBORS = 100  # Jumlah neighbors untuk ReliefF


# =============================================================================
# CUSTOM TRANSFORMER UNTUK ReliefF (agar kompatibel dengan Pipeline)
# =============================================================================
class ReliefFSelector(BaseEstimator, TransformerMixin):
    """
    Wrapper untuk ReliefF agar kompatibel dengan sklearn Pipeline.
    
    ReliefF dari skrebate tidak sepenuhnya kompatibel dengan Pipeline
    karena memerlukan y saat transform. Wrapper ini menyimpan hasil
    fitting dan menggunakannya untuk transform.
    """
    ...
    def fit(self, X, y):
        """Fit ReliefF dan simpan feature scores."""
        if RELIEFF_AVAILABLE:
            # Sesuaikan n_neighbors dengan jumlah samples
            ...
            # ReliefF memerlukan numpy array
            X_arr = np.array(X) if not isinstance(X, np.ndarray) else X
            y_arr = np.array(y) if not isinstance(y, np.ndarray) else y
            
            relieff.fit(X_arr, y_arr)
            
            self.feature_importances_ = relieff.feature_importances_
            
            # Dapatkan indices fitur terbaik
            ...
            # Fallback ke SelectKBest
            from sklearn.feature_selection import SelectKBest, f_classif
            selector = SelectKBest(f_classif, k=self.n_features_to_select)
            selector.fit(X, y)
            self.feature_importances_ = selector.scores_
    ...
    def transform(self, X):
        """Transform X dengan memilih fitur terbaik."""
        X_arr = np.array(X) if not isinstance(X, np.ndarray) else X
    ...
    def get_support(self, indices=False):
        """Dapatkan mask atau indices fitur terpilih."""
        if indices:
...
def get_models_and_params(n_features):
    ...
    Args:
        n_features: Jumlah fitur dalam dataset
...
def load_and_prepare_data(filepath, target_col=TARGET_COLUMN):
    ...
    if not RELIEFF_AVAILABLE:
        print("⚠️  ReliefF not available, using SelectKBest fallback")
    print("=" * 60)
...
def create_pipeline(model):
    """
    Membuat Pipeline dengan: Imputer -> Scaler -> ReliefF Selection -> Classifier
    ...
    pipeline = Pipeline([
        ...
        ('selector', ReliefFSelector(
            n_features_to_select=10,  # Default, akan di-override oleh GridSearch
            n_neighbors=RELIEFF_N_NEIGHBORS
        )),
        ('clf', model)
    ])
...
def train_with_gridsearch(pipeline, params, X_train, y_train, model_name):
    ...
    best_k = grid_search.best_params_.get('selector__n_features_to_select', 'N/A')
...
def plot_confusion_matrix(cm, model_name, output_path, test_size):
    ...
    sns.heatmap(
        ...
        annot=True, 
        fmt='d', 
        cmap='Purples',  # Warna berbeda untuk ReliefF
        xticklabels=['Not Passing (0)', 'Passing (1)'],
        yticklabels=['Not Passing (0)', 'Passing (1)']
    )
    plt.title(f'Confusion Matrix - {model_name}\n(ReliefF Selection, Test Size: {test_size*100:.0f}%)')
    plt.ylabel('Actual')
...
def plot_feature_importance(pipeline, feature_names, model_name, output_path, test_size):
    """Plot skor ReliefF untuk fitur."""
    try:
        selector = pipeline.named_steps['selector']
        scores = selector.feature_importances_
        
        if scores is None:
            print(f"  No feature scores available")
            return
        ...
        colors = ['purple' if s > 0 else 'red' for s in df_top['score'].values]
        bars = plt.barh(range(top_n), df_top['score'].values, color=colors)
        plt.yticks(range(top_n), df_top['feature'].values)
        plt.xlabel('ReliefF Score')
        plt.ylabel('Features')
        plt.title(f'Top {top_n} Features by ReliefF Score - {model_name}\n(Test Size: {test_size*100:.0f}%)')
...
def save_report(all_results, output_path, test_size, feature_names):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("HYPERPARAMETER TUNING WITH ReliefF FEATURE SELECTION\n")
        f.write("=" * 80 + "\n")
        f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Feature Selection Method: ReliefF\n")
        f.write(f"ReliefF Available: {RELIEFF_AVAILABLE}\n")
        ...
        for model_name, result in all_results.items():
            ...
            k = result['best_params'].get('selector__n_features_to_select', 'N/A')
...
def plot_comparison(all_results, output_path, test_size):
    ...
    colors = ['#9b59b6', '#3498db', '#e74c3c', '#f39c12']  # Warna berbeda
    ...
    ax.set_title(f'Model Comparison - ReliefF Feature Selection\n(Test Size: {test_size*100:.0f}%)', fontsize=14)
    ax.set_xticks(x)
...
def generate_final_summary(all_benchmarks, output_path, feature_names):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("FINAL SUMMARY - ReliefF FEATURE SELECTION BENCHMARK\n")
        f.write("=" * 80 + "\n")
        f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Feature Selection Method: ReliefF\n")
        f.write(f"ReliefF Package Available: {RELIEFF_AVAILABLE}\n")
        ...
        for test_size, results in all_benchmarks.items():
            ...
            k = best_model[1]['best_params'].get('selector__n_features_to_select', 'N/A')
...
def main():
    ...
    if not RELIEFF_AVAILABLE:
        ...
        print("   pip install skrebate")
        print("   Proceeding with SelectKBest fallback...\n")
